...
# VIEW: DETAILS
    ...
    # Fetch genre-based recommendations using the NEW endpoint
    genre_recs, genre_err = api_get_json(
        f"/movie/recommend/genre/{tmdb_id}",
        params={"limit": 12}
    )

    if not genre_err and genre_recs:
        st.markdown("#### 🎭 More Like This (Genre)")
        poster_grid(
            genre_recs,  # 👈 Already in correct card format from backend
            cols=grid_cols,
            key_prefix="details_genre"
        )
    else:
        st.warning(f"Failed to load recommendations: {genre_err or 'No data returned'}")
